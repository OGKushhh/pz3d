...
# that's just below water level, encouraging flooding visuals).
const TERRAIN_HEIGHT_VERSION := 3

# Per-biome elevation signatures (GDD §12.4)
# NOTE: RIVER (was key 6) removed in Phase A.4 — the river carve is layered
# on top of whatever biome exists at (x, z) via the _river.distance_to() call
# in height_at(). This means a river in FOREST has +2m banks + -4m carve =
# -2m riverbed (visually deeper), while a river in FARMLAND has +0.2m banks
# + -4m carve = -3.8m riverbed. Each biome's river looks slightly different,
