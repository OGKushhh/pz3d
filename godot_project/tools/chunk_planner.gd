...
static func plan_chunk(
	...
	# Track per-biome per-type counts for anti-repetition
	...
	for parcel in parcels:
		...
		# Lot recipe selection (renderer handles actual stamping)
		# For now, planner just records that a lot should go here
		# The renderer will call lot_stamper.pick_lot_for_biome + stamp_lot
		if crng.randf() <= fill:
			plan["buildings"].append({
				"pos": lot_pos,
				"rot_y": 0.0,  # will be set by renderer based on front_dir
				"asset_name": "lot_recipe",  # renderer picks the actual recipe
				"parcel_id": parcel.parcel_id,
				"is_landmark": false,
				"is_lot": true,
			})
