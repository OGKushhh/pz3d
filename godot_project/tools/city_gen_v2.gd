...
static func plan_block(block: Dictionary, seed: int) -> Dictionary:
        ...
        # Pick a recipe for this block — pass block coords so adjacent blocks get different recipes
        var recipe_name: String = BlockRecipes.pick_recipe(district, seed + int(center.x) * 31 + int(center.z) * 17)
